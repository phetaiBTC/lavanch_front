<template>
  ...
  >
    ...
    <!-- Category name -->
    <Column style="min-width: 180px" :header="$t('category')" sortable>
      <template #body="{ data }">
        {{ data.category?.name || "-" }}
      </template>
    </Column>
...
const props = defineProps<{
  data: PaginatedResponse<IProductEntity>;
  value: IProductEntity[];
  title: string;
  loading: boolean;
  sort?: sortType;
  checked?: Status;
...
const selection = ref<IProductEntity[]>(props.value);
