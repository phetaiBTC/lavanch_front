<template>
  ...
  >
    ...
    <!-- Product Variant -->
    <Column :header="$t('product_variant')" style="min-width: 200px">
      <template #body="{ data }">
    ...
    <!-- Unit -->
    <Column :header="$t('unit')" style="min-width: 150px">
      <template #body="{ data }">
        {{ data.unit?.name || "-" }}
      </template>
    </Column>
    ...
    <Column
      ...
      :header="$t('quantity_per_unit')"
      :sortable="true"
      style="min-width: 150px"
    />

    <!-- Base Unit -->
    <Column field="is_base_unit" :header="$t('is_base_unit')" style="min-width: 120px">
      <template #body="{ data }">
        <Tag
          :value="data.is_base_unit ? $t('yes') : $t('no')"
          :severity="data.is_base_unit ? 'success' : 'warning'"
        />
      </template>
    </Column>
...
const props = defineProps<{
  data: PaginatedResponse<IProductUnit>;
  value: IProductUnit[];
  title: string;
  loading: boolean;
  sort?: sortType;
...
const selection = ref<IProductUnit[]>(props.value);
